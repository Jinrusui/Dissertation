...
class RunWithWorldModel(BaseRun):
    ...
    def __init__(self, env_name, prompt, model, output_dir="./experiments/", 
        ...
        BaseRun.Agent = partial(VLMWorldModelAgent, tokenizer=self.tokenizer, world_model=self.world_model,plan_horizon=self.plan_horizon, num_plans=self.num_plans, device=self.device)
